...
import { NcAppContent, NcNoteCard, NcContent, NcButton, NcEmptyContent, NcIconSvgWrapper } from '@nextcloud/vue'
...
const copyButtonIcon = computed(() => {
  if (copyState.value === 'success') { return IconCheckmark }
  if (copyState.value === 'error') { return IconError }
  return IconCopy
})

const downloadButtonIcon = computed(() => {
  if (downloadState.value === 'success') { return IconCheckmark }
  if (downloadState.value === 'error') { return IconError }
  return IconDownload
})
...
async function downloadAsFile(content) {
  try {
...
async function loadSecret() {
  ...
  try {
    ...
    const key = await $cryptolib.importDecryptionKey(window.location.hash.substring(1), iv)
...
<template>
  <NcContent app-name="secrets">
		<NcAppContent class="centered">
			<h2 class="centered">{{ t('secrets', 'The following secret has been shared with you securely:') }}</h2>
