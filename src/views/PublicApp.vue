...
import { NcAppContent, NcButton, NcContent, NcEmptyContent, NcIconSvgWrapper, NcNoteCard, NcTextArea } from '@nextcloud/vue'
...
import SecretRetrievalButtons from '@/components/SecretRetrievalButtons.vue'
...
const hasBeenDestroyed = ref<boolean>(false)
const hasBeenRetrieved = ref<boolean>(false)
...
window.onbeforeunload = function() {
	if (hasBeenDestroyed.value && !hasBeenRetrieved.value) {
		return t('secrets', 'Are you sure you want to leave the page? You will not be able to retrieve the secret again.')
	}
	return undefined
}
</script>
...
<template>
	<NcContent v-if="isSecureContext" appName="secrets">
		<NcAppContent :class="$style.content">
			<h5 class="mobile-text-centered">{{ t('secrets', 'The following secret has been shared with you securely:') }}</h5>
			...
			<div class="secret-container">
				...
				<SecretRetrievalButtons
					v-if="decrypted"
					class="mobile-flex-1"
					:decrypted="decrypted"
					:secretFilename="secretFilename"
					@secretRetrieved="hasBeenRetrieved = true" />
				<NcTextArea
					v-if="decrypted"
...
<style scoped>
  ...
  div.secret-container>.secret-content {
    margin-top: 0;
  }
