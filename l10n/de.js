OC.L10N.register(
    "secrets",
    {
    "A <strong>Secret</strong> was created" : "Ein <strong>Geheimnis</strong> wurde erstellt",
    "A <strong>Secret</strong> has expired without being retrieved" : "Ein <strong>Geheimnis</strong> ist abgelaufen, ohne dass es abgerufen wurde",
    "Secrets" : "Geheimnisse",
    "Secret '{secret}' has been retrieved" : "Das Geheimnis \"{secret}\" wurde abgerufen",
    "Secret '{secret}' has expired without being retrieved" : "Geheimnis \"{secret}\" ist abgelaufen, ohne dass es abgerufen wurde",
    "A <strong>Secret</strong> was retrieved" : "Ein <strong>Geheimnis</strong> wurde abgerufen",
    "Securely share data via link" : "Daten sicher via Link freigeben",
    "Secrets allows users to generate share links for text based data (e.g. passwords, CSV lists, bank accounts...) that can be sent to anyone (including receivers without a Nextcloud account) for retrieval. The data itself will be end-to-end encrypted (so not even Nextcloud can access it) and the encryption key will be part of the share link (the anchor part) - but never be actually sent to the server. Once retrieved, the secret will be deleted from the server, ensuring, that if it arrived at the correct receiver it has been seen by nobody else.\n\nSecrets now comes with a cli that can be used to automate the provisioning of secret shares: https://github.com/theCalcaholic/nextcloud-secrets/releases" : "Mit Secrets können Benutzer Freigabelinks für textbasierte Daten (z. B. Passwörter, CSV-Listen, Bankkonten usw.) generieren, die zum Abrufen an jeden gesendet werden können (auch an Empfänger ohne Nextcloud-Konto). Die Daten selbst werden Ende-zu-Ende verschlüsselt (so dass nicht einmal Nextcloud darauf zugreifen kann) und der Verschlüsselungsschlüssel ist Teil des Freigabelinks (der Ankerteil), wird aber nie tatsächlich an den Server gesendet. Nach dem Abrufen wird das Geheimnis vom Server gelöscht, sodass sichergestellt ist, dass es, wenn es beim richtigen Empfänger angekommen ist, von niemand anderem gesehen wurde.\n\nSecrets verfügt jetzt über ein Kommandozeileninterface, mit dem die Bereitstellung von geheimen Freigaben automatisiert werden kann: https://github.com/theCalcaholic/nextcloud-secrets/releases",
    "Expires on:" : "Ablaufdatum:",
    "Expiration Date" : "Ablaufdatum",
    "access password (optional)" : "Zugriffspasswort (Optional)",
    "If you set an access password, anyone opening the share link will also need to enter this password before they can view the secret." : "Wenn du ein Zugriffspasswort festlegst, muss jeder, der den Freigabelink öffnet, dieses Passwort eingeben, bevor er das Geheimnis einsehen kann.",
    "Secret content" : "Geheimer Inhalt",
    "Type or paste the secret you want to share (e.g. a password, CSV data, or bank account details)…" : "Gebe oder füge das Geheimnis ein, das du weitergeben möchtest (z. B. ein Passwort, CSV-Daten oder Bankkontodaten) …",
    "Save" : "Speichern",
    "No secret selected" : "Kein Geheimnis ausgewählt",
    "never" : "Niemals",
    "has access password" : "Hat Zugriffspasswort",
    "no access password" : "Kein Zugriffspasswort",
    "Share Link:" : "Freigabe-Link",
    "This secret has expired and its content was consequently deleted from the server." : "Dieses Geheimnis ist abgelaufen und sein Inhalt wurde folglich vom Server gelöscht.",
    "This secret has already been retrieved and its content was consequently deleted from the server." : "Dieses Geheimnis wurde bereits abgerufen und sein Inhalt wurde folglich vom Server gelöscht.",
    "Could not decrypt secret (key not available locally)." : "Geheimnis konnte nicht entschlüsselt werden (Schlüssel lokal nicht verfügbar).",
    "_Will be deleted in %n day_::_Will be deleted in %n days_" : ["Wird in %n Tag gelöscht","Wird in %n Tagen gelöscht"],
    "Could not fetch secrets" : "Abruf der Geheimnisse fehlgeschlagen",
    "Can't save already existing secret" : "Bereits vorhandenes Geheimnis kann nicht gespeichert werden",
    "Secret from {date} {time}" : "Geheime Zeichenkette vom {date} {time}",
    "Could not create the secret" : "Erstellung des Geheimnisses fehlgeschlagen",
    "Could not delete the secret" : "Löschen des Geheimnisses fehlgeschlagen",
    "Secret deleted" : "Geheimnis gelöscht",
    "Navigation" : "Navigation",
    "New secret" : "Neues Geheimnis",
    "Change Title" : "Titel ändern",
    "Cancel secret creation" : "Geheimniserstellung abbrechen",
    "Delete secret" : "Geheimnis löschen",
    "Your secret is stored end-to-end encrypted on the server. It can only be decrypted by someone who has been given the link. Once retrieved successfully, the secret will be deleted on the server" : "Dein Geheimnis wird Ende-zu-Ende verschlüsselt auf dem Server gespeichert. Es kann nur von jemandem entschlüsselt werden, der den Link erhalten hat. Nach erfolgreichem Abruf wird das Geheimnis auf dem Server gelöscht",
    "Create a secret to get started" : "Erstelle ein Geheimnis, um loszulegen",
    "Secrets is only available when visiting Nextcloud at an encrypted (https) address." : "„Secrets“ ist nur verfügbar, wenn Nextcloud über eine verschlüsselte (https) Adresse aufgerufen wird.",
    "Could not decrypt secret" : "Geheimnis konnte nicht entschlüsselt werden",
    "Are you sure you want to leave the page? You will not be able to retrieve the secret again." : "Diese Seite verlassen? Du kannst das Geheimnis danach nicht erneut abrufen.",
    "The following secret has been shared with you securely:" : "Das folgende Geheimnis wurde dir auf sicherem Weg mitgeteilt:",
    "Please make sure you have copied and stored the secret before closing this page! It is now deleted on the server." : "Bitte stelle sicher, dass du das Geheimnis kopiert und gespeichert hast, bevor du diese Seite schließt! Es ist nun auf dem Server gelöscht.",
    "Copy to Clipboard" : "In die Zwischenablage kopieren",
    "Copy the secret to the clipboard" : "Geheimnis in die Zwischenablage kopieren",
    "Download" : "Herunterladen",
    "Download the secret as a file" : "Geheimnis als Datei herunterladen",
    "Retrieving secret…" : "Geheimnis abrufen …",
    "Error loading secret. Is your link correct?" : "Laden des Geheimnisses fehlgeschlagen. Ist dein Link richtig?",
    "Revealing will delete the secret from the server. You will not be able to retrieve it again." : "Durch die Offenlegung wird das Geheimnis vom Server gelöscht. Es kann dann nicht mehr abgerufen werden.",
    "Reveal the secret and delete it on the server" : "Das Geheimnis aufdecken und auf dem Server löschen",
    "I understand. Reveal and destroy Secret." : "Verstanden. Geheimnis offenlegen und löschen.",
    "This share is password-protected" : "Diese Freigabe ist passwortgeschützt",
    "The password is wrong or has expired. Please try again." : "Das Passwort ist falsch oder abgelaufen. Bitte noch einmal versuchen.",
    "Password" : "Passwort",
    "This path has changed. Redirecting you to the new location. If this doesn't work, click the following link:" : "Dieser Pfad hat sich geändert. Du wirst zum neuen Speicherort weitergeleitet. Wenn dies nicht funktioniert, auf folgenden Link klicken:",
    "Go to secret" : "Zum Geheimnis wechseln",
    "Securely share data with anyone. All data is end-to-end encrypted by the user and will be deleted once retrieved successfully" : "Teile Daten sicher mit jedem. Alle Daten werden vom Benutzer Ende-zu-Ende verschlüsselt und nach erfolgreichem Abruf gelöscht",
    "New Secret" : "Neues Geheimnis",
    "Share Link" : "Link teilen",
    "password protected" : "Passwortgeschützt",
    "Copy Secret Link" : "Geheimen Link kopieren",
    "share password (optional)" : "Passwort teilen (Optional)",
    "Copy the secret's content to the clipboard" : "Den Inhalt des Geheimnisses in die Zwischenablage kopieren",
    "Download the secret's content as a file" : "Den Inhalt des Geheimnisses als Datei herunterladen",
    "Retrieving secret..." : "Geheimnis abrufen …"
},
"nplurals=2; plural=(n != 1);");
